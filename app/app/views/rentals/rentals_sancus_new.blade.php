	...
	@section('content')
		...
		<div class="panel panel-default">
			  ...
			  <div class="panel-body">				
					...
					{{Form::open(array('url'=>'additem_sancus_rentals', 'id'=>'form_add_item'))}}
					    ...
					    <div class="panel panel-default">
					      ...
					      <div id="2" class="panel-collapse collapse in " role="tabpanel" aria-labelledby="headingTwo">
					        <div class="panel-body">    
					        	@if(Session::has('data'))
					        		<label>Desde</label>
					        		<input type="text" value="{{Session::get('data')['date']}}" class="form-control" disabled><br>
					        		<label>Hasta</label>
					        		<input type="text" value="{{Session::get('data')['to']}}"  class="form-control" disabled>
					        	@else
					        		<label>Desde</label>
					          		{{ Form::date('date') }} <br>  
					          		<label>Hasta</label>
					           ...
					           <br>
					         	...
					         	<br>
					         	<input class="form-control" placeholder='Medico Referencia' id="doctor_name" value="{{Session::get('data')['doctor_name']}}"  >
					         	<input type="hidden" name="doctor_id" id="doctor_id" data-id="">
					    	...
					    	<tbody id='table_items_body'>
					    		@if(Session::has('array_items'))
					    			...
					    			<tr>
					    				...
					    				<td> $ {{$key['$']}}</td>
					    				<td> $ {{$key['subtotal'] }}</td>	
					    				<td><a href="{{route('rentals_delitem', $item )}}" class="del_confirm pull-right"><i class="fa fa-remove"></i></a>
					    				</td>	    			
					    			</tr>
					    			@endforeach
					    		@endif
					...
					<hr>		 
					  <a href="{{route('rentals_cancel')}}" id="cancel" class="del btn btn-danger">Cancelar</a>
	...
	@section('js')
		<script type="text/javascript">
				...
				$('#process').on('click',function()
				{
					$.ajax({
						type: "GET",
						url: "process_rentals_sancus",
						dataType: "json",
						  success: function(data){
								if(data != null)
								{
									window.location.href = "inicio";
									window.open('remito_rentals/'+data , '_blank');	
			     ...
			     $("#client_name").autocomplete({      
			  	 ...
			  	 $("#doctor_name").autocomplete({      
					...
					$('#doctor_id').attr('data-id',ui.item.id);
					}
			  	 });
				...
				*/
				...
				$("#item").autocomplete({
                    ...
                    select: function(event, ui) {
                   		...
                   		$('#price').val(ui.item.rental_price);
				...
				$('#del_item').on('click',function(){
 				...
 				$('#add_item').on('click',function(){
 					...
 					if($('#client_name').val() == "" || $('#doctor_name').val() == "" )
 					{
