...
<table class="header">
  <tbody>
    <tr>
      ...
      <td width="40%"><h2 style="text-align: right">Venta : 000-0{{$sales->id}}</h2></td>
    </tr>
  </tbody>
</table>
...
<table style="width: 100%; border-top: 1px solid black; border-bottom: 1px solid black; font-size: 8pt;">
  <tbody>
    ...
    <br>
      <td>
        Nombre Apellido: <strong>{{$sales->Clients->name}} {{$sales->Clients->name }} </strong>
        <br>
        Cuit/Cuil/Dni :  <strong>{{$sales->Clients->dni}} / {{$sales->Clients->cuit }} </strong>
        <br>
        Email :  <strong>{{$sales->Clients->email}} </strong>
      </td>
      <td>
      Empresa: <strong>{{$sales->Clients->company_name}} </strong>
      <br>
       Dirección: <strong>{{$sales->Clients->address}} </strong>
       <br>
        Tel./Cel.: <strong>{{$sales->Clients->phone}} / {{$sales->Clients->cell_phone}} </strong>
      </td>
    </tr>
  <br>
  </tbody>
