...
<table class="header">
  <tbody>
    <tr>
      ...
      <td width="40%"><h2 style="text-align: right">Alquiler : 000-0{{$rentals->id}}</h2></td>
    </tr>
  </tbody>
</table>
...
<table style="width: 100%; border-top: 1px solid black; border-bottom: 1px solid black; font-size: 8pt;">
  <tbody>
    ...
    <br>
      <td>
        Nombre Apellido: <strong>{{$rentals->Clients->name}} {{$rentals->Clients->name }} </strong>
        <br>
        Cuit/Cuil/Dni :  <strong>{{$rentals->Clients->dni}} / {{$rentals->Clients->cuit }} </strong>
        <br>
        Email :  <strong>{{$rentals->Clients->email}} </strong>
      </td>
      <td>
      Empresa: <strong>{{$rentals->Clients->company_name}} </strong>
      <br>
       Dirección: <strong>{{$rentals->Clients->address}} </strong>
       <br>
        Tel./Cel.: <strong>{{$rentals->Clients->phone}} / {{$rentals->Clients->cell_phone}} </strong>
      </td>
    </tr>
  <br>
  </tbody>
...
<table class="items">
  ...
  <tbody>
    <tr>
      <td colspan="6">
      <h3>Alquiler:</h3>
      Del <strong>{{$rentals->rentals_date}}</strong>
      al <strong>{{$rentals->rentals_to}}</strong>
      </td>
    </tr>
  </tbody>
  ...
  <tbody>
    ...
    @foreach($rentals->RentalsItems as $item )
      @if($cont % 2 == 0 )
         <tr class="even_row">
      @else
         <tr class="odd_row">
  ...
  <tbody>
    <tr>
      ...
      <td style="text-align: center"><strong>$ {{$rentals->amount}}</strong></td>
    </tr>
  </tbody>
