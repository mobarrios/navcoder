<?php
    ...
    Form::macro('currency', function($name, $label)
    {
        $curr    = array('AR$'=>'AR$','U$D'=>'U$D', '€'=>'€');
        $input   = Form::select($name ,  $curr  , null , array('class'=>'form-control')); 

        return buildInput($input,$label);  
    });
    
    Form::macro('rooms', function($name, $label)
    {
        $types   = Rooms::lists('name','id');
        $input   = Form::select($name , array('Ninguno') + $types ,null, array('class'=>'form-control')); 

        return buildInput($input,$label);  
    });

    Form::macro('types', function($name, $label)
        {
    ...
    Form::macro('providers', function($name, $label)
 	...
 	Form::macro('date', function($name, $label)
        {
            $value = Form::getValueAttribute($name);
            
            $input =  '<input type="text" class="datepicker form-control " name="'.$name.'" placeholder="dd-mm-aaaa" value="'.$value.'">';
